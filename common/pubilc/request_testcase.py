...
class TestLogin(unittest.TestCase):
    ...
    def test_2purchase_order(self, test_data):
        ...
        print("cookie的值是：{0},类型是{1}".format(getattr(GetData, test_data["role_cookie"]), type(getattr(GetData, test_data["role_cookie"]))))
        my_logger.info_log(str(getattr(GetData, test_data["role_cookie"])))
        res = HttpRequest(test_data["url"],json.loads(test_data["data"]),test_data["method"],eval(test_data["headers"])).http_request(getattr(GetData,test_data["role_cookie"]))


        #生成报备单后，取到报备单id
        if test_data["url"].find("pretrade/report/add") != -1:
            setattr(GetData,"reportID", DoSql().do_sql("SELECT reportId FROM yy_report as a LEFT JOIN yy_customer as b on a.customerId = b.customerId where b.`name` = '{0}'  ORDER BY a.reportTime DESC LIMIT 1 ;".format(getattr(GetData,"customName"))))
    ...
    def test_3rec_comm_ticket(self,rec_data):
        ...
        setattr(GetData, "receivable_commision", DoSql().do_sql("SELECT receivable_commision from yy_purchase_order where id = {0};".format(str(getattr(GetData, "tradeId")))))
        ...
        if rec_data["data"].find("@receivable_commision@") != -1:
            rec_data["data"] = rec_data["data"].replace("@receivable_commision@", str(getattr(GetData, "receivable_commision")))

        res = HttpRequest(rec_data["url"],json.loads(rec_data["data"]),rec_data["method"],eval(rec_data["headers"])).http_request(getattr(GetData,rec_data["role_cookie"]))
        TestResult = ""
        # 断言
        if rec_data["url"].find("commission/ticket/manual/create") != -1:
            setattr(GetData,"rec_ticketId",DoSql().do_sql("SELECT ticket_id from yy_rec_comm_ticket_order where order_id = '{0}'".format(getattr(GetData,"tradeId"))))
        try:
    ...
    def test_4pay_comm_ticket(self,pay_data):
        ...
        if pay_data["data"].find("@payable_commission@") != -1:
            pay_data["data"] = pay_data["data"].replace("@payable_commission@", str(getattr(GetData, "payable_commission")))

        if pay_data["data"].find("@receivable_commision@") != -1:
            pay_data["data"] = pay_data["data"].replace("@receivable_commision@", str(getattr(GetData, "receivable_commision")))
